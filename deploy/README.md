...
```
push 到 main
  └─ GitHub Actions: npm ci → npm run test → tsc --noEmit
...
- **不要给部署用户 sudo。** 当前架构不需要。若某个改动"需要 sudo 才能做"，
  多半是架构走偏了，先回来讨论。
- **不要把主机地址、凭据、证书路径写进仓库。**
...
同机已有服务的形态是「uvicorn 监听 127.0.0.1 高位端口 + nginx `proxy_pass` 反代」，
新服务照此即可。但**在写代码之前**必须先做两件事：

1. 在 `docs/PRODUCT_CONCEPT.md` 记录后端技术栈选型决定，并把对应条目从末尾的
   「未决问题」清单中移除。`AGENTS.md` 禁止在选型定下来之前引入相关框架依赖。
2. 补齐 §14 隔离要求里当前用不上的几条：独立系统用户（不要复用静态站的部署账号）、
   独立数据库、进程内存上限、反向代理限流。

LLM 密钥只存在于服务端的 `.env`（已 gitignore），`.env.example` 只放安全占位符。
