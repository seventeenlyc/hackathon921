...
音频资源位于 `public/audio/`，构建后原样出现在 `dist/audio/`。前 200 波的背景曲目放在 `background/` 目录，开发服务器启动和生产构建时自动收集该目录中的 MP3、OGG、WAV、M4A 文件；新增曲目后需重新启动开发服务器或重新构建。`wave.wav` 与 `game-over.wav` 是活动音效；替换资源时使用浏览器可播放的格式，并在替换前确认许可归属。旧的 `background.wav` 仅作为历史占位文件保留，不是活动背景音乐。资源请求或解码失败必须仍不阻塞游戏。
