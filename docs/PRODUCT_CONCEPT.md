...
这是一台**共享生产机**，已运行 nginx（7 个站点，均配 Let's Encrypt 证书）、PostgreSQL 13 与多个 Python 服务。规格为 2 vCPU / 1.8 GB 内存 / 40 GB 磁盘，负载长期处于空闲，30 天内无 OOM 记录。

由此产生两条硬约束：

- **不得在该服务器上执行前端构建。** 内存余量约 1.1 GB，构建峰值可能将生产服务挤入 swap。构建应在本地或 CI 完成，仅上传产物。
- **新服务必须与既有生产业务隔离**：独立系统用户、独立数据库、systemd 内存上限、nginx 限流。
