...
> **当前实现（2026-09，过渡阶段）**：线上尚无后端，且 AGENTS.md 禁止上线可通过客户端伪造成绩的排行榜接口。因此当前先交付**纯前端本地排行榜**：首次进入时提示输入用户名（写入 cookie），排行榜按「到达的最高波次」排序，界面左下角显示前十与当前用户的排名，登上前十高亮；数据存于本地（localStorage / cookie）。跨设备与跨用户的真实共享排行榜仍需先完成后端技术选型与服务端对局校验设计（见文末「身份与后端」「排行榜」未决问题），届时用服务端实现替换本地实现。
