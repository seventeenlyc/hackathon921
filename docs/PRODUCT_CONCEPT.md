...
3. 波次持续进行，并越来越难。
4. 玩家观察战场与 AI 的行为。
5. 在该局仍在进行时，玩家可以 PAUSE，暂停并保留全部战场状态，修改策略 Prompt 后 RESUME。
6. 更新后的策略影响后续的 AI 决策。
7. 这个过程持续到基地被摧毁。
8. 玩家的得分是该局中到达的最高波次。
...
MVP 中，玩家可以随时 PAUSE、修改 Prompt、RESUME，改动不消耗资源，也不重新开始游戏——波次、金币、基地生命、炮台、敌人与地图状态全部保留，只有后续 AI 决策发生变化。游戏在**正常速度与加速运行之间可切换**，方便玩家在不修改策略时更快地让策略接受后续波次的检验。是否在未来引入冷却、次数或资源成本尚未确定。
